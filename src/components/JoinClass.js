...
function JoinClass() {
  ...
  const joinClass = async () => {
    try {
      ...
      if (!classRef.exists) {
        return alert(`Este grupo não existe, por favor verifique a ID`);
      }
      ...
      // alert done
      alert(`Você entrou no grupo  ${classData.name} com sucesso!`);
      handleClose();
    } catch (err) {
      console.error(err);
      alert(err.message);
  ...
  return (
    <div className="joinClass">
      ...
      >
        <DialogTitle id="form-dialog-title">Entrar no grupo</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Ensira o ID do grupo que deseja entrar
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
